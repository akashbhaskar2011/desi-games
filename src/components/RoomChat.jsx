...
function ChatPanel({
  ...
  return (
    <section
      className={`room-chat ${drawer ? "room-chat-drawer" : ""}`}
    ...
    >
      ...
      <div className="room-chat-messages" aria-live="polite">
        {messages.length === 0 && (
          <p className="room-chat-empty">Say hello to your opponent.</p>
        )}
        {messages.map((item) => (
          <article
            className={`chat-message ${item.playerId === playerId ? "chat-message-own" : ""}`}
            key={item.id}
          >
            <strong>
              {item.playerId === playerId ? "You" : item.playerName}
            </strong>
            <p>{item.message}</p>
            <time>
...
export function RoomChat({ messages, playerId, disabled, error, onSend }) {
  ...
  return (
    <>
      <div className="room-chat-desktop">
        <ChatPanel
          messages={messages}
          playerId={playerId}
          disabled={disabled}
      ...
      <button className="room-chat-toggle" onClick={() => setOpen(true)}>
        <MessageCircle size={17} /> Chat
        {messages.length ? <span>{messages.length}</span> : null}
      </button>
      {open && (
        <div className="room-chat-backdrop">
          <ChatPanel
            drawer
            messages={messages}
            playerId={playerId}
