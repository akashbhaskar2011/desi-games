...
export function WaitingRoom({ room, connection, error }) {
  ...
  async function start() {
    ...
    setStartError("");
    try {
      await apiRequest(`/api/rooms/${room.roomCode}/start`, {
        method: "POST",
  ...
  return (
    <section className="page-width page-section waiting-page">
      ...
      {error && <p className="form-error room-alert">{error}</p>}
      <div className="waiting-card">
        <div className="waiting-card-head">
          <RoomCode code={room.roomCode} />
          <div>
            <span className="room-game-label">GAME</span>
            <strong>
              {room.gameId === "barah-goti" ? "Barah Goti" : room.gameId}
            </strong>
          </div>
        </div>
        <div className="waiting-card-body">
          <div className="player-heading">
            <h2>
              Players <span>{connectedPlayers}/4</span>
            </h2>
            <span>
              {connectedPlayers < 2
          ...
          <ShareButtons roomCode={room.roomCode} gameName="Barah Goti" />
          <div className="host-actions">
            {isHost && (
              <button
                className="button button-primary"
                onClick={start}
                disabled={starting || connectedPlayers < 2}
              >
                {starting ? (
