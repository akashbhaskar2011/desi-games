...
export function StopMatchControl({ className = '', roomCode, playerId }) {
  ...
  async function stop() { if (roomCode && playerId) { await apiRequest(`/api/rooms/${roomCode}/terminate`, { method: 'POST', body: JSON.stringify({ playerId, reason: 'match_stopped' }) }); clearActiveMatchForRoom(roomCode) }; setOpen(false) }
