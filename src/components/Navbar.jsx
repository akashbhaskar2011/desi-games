  ...
  return <header className="site-nav">
    <div className="nav-inner">
      ...
      <nav className={`nav-links ${open ? 'nav-links-open' : ''}`} aria-label="Main navigation">
        ...
        {match && !isViewingMatch(location.pathname, match.roomCode) && <NavLink className="nav-continue" to={`/play/${match.roomCode}`} onClick={() => setOpen(false)}>🎮 Continue Match{activeGame ? ` · ${activeGame.name}` : ''}</NavLink>}
