...
export function legalMoves(board, node, role) {
  ...
  const moves = BOARD_NODES.filter(
    (candidate) =>
      candidate.id !== node &&
      Math.abs(candidate.row - current.row) <= 1 &&
      Math.abs(candidate.col - current.col) <= 1 &&
      board[candidate.id] === null,
  ).map((candidate) => candidate.id);
  if (role === "TIGER")
    BOARD_NODES.forEach((candidate) => {
      if (captureNode(board, node, candidate.id) !== null)
        moves.push(candidate.id);
    });
