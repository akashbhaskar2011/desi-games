...
export function useGame(roomCode) {
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      ...
      updateCurrentRoom({ lastConnection: "connected", roomStatus: "PLAYING" });
      socket.emit("room:join", { roomCode, playerId });
      socket.emit("game:request-state", { roomCode });
    });
    socket.on("disconnect", () => {
      setConnection("reconnecting");
      setError("Reconnecting to your match…");
    });
    socket.on("connect_error", () => {
      setConnection("disconnected");
      setError("Unable to reconnect to this match.");
    });
    socket.on("game:state", (nextState) => {
      setState(nextState);
      if (
        ["terminated", "finished", "completed", "expired"].includes(
          nextState.status,
        )
      )
    ...
    socket.on("game:terminated", ({ reason }) => {
      setState((current) =>
        current
          ? { ...current, status: "terminated", terminationReason: reason }
          : current,
      );
      clearActiveMatchForRoom(roomCode);
    });
    socket.on("room:error", (payload) => {
      ...
      setError(payload.message || "This match is no longer available.");
    });
    socket.on("game:error", (payload) => {
      if (payload.code === "ROOM_NOT_FOUND") clearActiveMatchForRoom(roomCode);
      setError(payload.message || "Unable to recover this match.");
    });
    ...
    socket.on("chat:error", (payload) =>
      setChatError(payload.message || "Unable to send that message."),
    );
    return () => {
      socket.disconnect();
