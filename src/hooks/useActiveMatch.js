  ...
  useEffect(() => {
    ...
    const refresh = () => {
      ...
      if (local)
        verifyActiveMatch()
          .then((verified) => {
            if (alive) setMatch(verified);
          })
          .catch((error) => {
            if (error.status === 404 && alive) setMatch(null);
          });
    };
    refresh();
    window.addEventListener("desi-active-match-change", refresh);
    window.addEventListener("storage", refresh);
    return () => {
      alive = false;
      window.removeEventListener("desi-active-match-change", refresh);
      window.removeEventListener("storage", refresh);
    };
  }, []);
  const value = useMemo(
    () => ({ match, setMatch: (patch) => setMatch(updateActiveMatch(patch)) }),
    [match],
  );
