...
export function useRoom(roomCode) {
  ...
  useEffect(() => {
    ...
    socket.on('connect', () => { setConnection('connected'); updateCurrentRoom({ roomStatus: 'WAITING' }); socket.emit('room:join', { roomCode, playerId }) })
    socket.on('disconnect', () => setConnection('reconnecting'))
    socket.on('connect_error', () => { setConnection('reconnecting'); setError('Connection lost. Reconnecting…') })
    socket.on('room:update', (nextRoom) => { setRoom(nextRoom); updateCurrentRoom({ gameId: nextRoom.gameId, players: nextRoom.players, roomStatus: nextRoom.status }) })
    socket.on('room:error', (payload) => { if (payload.code === 'ROOM_NOT_FOUND') clearActiveMatchForRoom(roomCode); setError(payload.message || 'Unable to enter this room.') })
